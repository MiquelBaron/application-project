...
from appointment.core.date_time import combine_date_and_time, get_weekday_num
...
# Calcula los slots dado un staff y un dia
def calculate_staff_slots(date, staff_member):
    ...
    weekday_num = get_weekday_num_from_date(date) #Obtenir numero de la setmana
    ...
    #Obtenim datetime de inici i fi de la jornada labroal del staff aquell dia
    ...
    # Obntenir el temps minim que ha de passar des de ara fins poder reservar (configurable per staff sino fallback de conf)
    ...
    # Tornem a obtenir la durada dels slots
    ...
    # A partir de toda esta info, calcula los slots
...
def get_user_by_telephone(telephone):
    if not isinstance(telephone, str):
        return JsonResponse({"error":"Telephone number must be a string."}, status=400)
    try:
        client = Client.objects.get(phone_number=telephone)
    except Client.DoesNotExist:
        logger.error(f"There is no client associated with this number:{telephone}")
        return JsonResponse({"error":f"There is no client associated with this number:{telephone}"}, status=400)


def create_and_save_appointment(appointment_data: dict):
    ...
    try:
        ...
        logger.error("Error creating and saving appointment.")
        logger.error(e)
        return JsonResponse({"Eror":"Error while creating the appointment."})
...
# Obtenir les cites d'un staff donada una data i una hora d'inici o final.
def get_appointments_for_date_and_time(date, start_time, end_time, staff_member):
    ...
    """
    object = Appointment.objects.filter(
            date=date,
...
def get_staff_member_appointment_list(staff_member: StaffMember) -> list:
    ...
    return Appointment.objects.filter(staff_member=staff_member)
