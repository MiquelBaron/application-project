...
from appointment.notifications import tasks_email
...
def test_enqueue_confirmation_email_only_if_client_has_email(monkeypatch):
    ...
    monkeypatch.setattr(email_service.send_appointment_confirmation_email_task, "delay", fake_delay)
...
def test_enqueue_confirmation_email_respects_settings(monkeypatch, settings, enabled):
    ...
    assert len(called) == (1 if enabled else 0)
...
def test_send_confirmation_email_calls_resend(monkeypatch, settings):
    ...
    monkeypatch.setattr(tasks_email.requests, "post", fake_post)

    assert tasks_email.send_appointment_confirmation_email_task.run(appointment.id) is True
